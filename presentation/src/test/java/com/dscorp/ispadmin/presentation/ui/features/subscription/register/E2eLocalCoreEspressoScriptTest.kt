...
class E2eLocalCoreEspressoScriptTest {
    ...
    fun `local core espresso script talks to Core via emulator reverse not staging or prod`() {
        ...
        assertThat(text).contains("E2E_WIFI_SSID=\"\${CLI_WIFI_SSID:-\${E2E_WIFI_SSID:-\$_E2E_WIFI_SSID_DEFAULT}}\"")
        assertThat(text).contains("_E2E_WIFI_SSID_DEFAULT=\"mimiwifi\"")
        assertThat(text).contains("lab-vsol-e2e-24")
        assertThat(prod).contains("E2E_USER=\"\${E2E_USER:-dscorp}\"")
        assertThat(prod).contains("E2E_PASSWORD=\"\${E2E_PASSWORD:-nohacker}\"")
        assertThat(prod).contains("E2E_WIFI_SSID=\"\${CLI_WIFI_SSID:-\${E2E_WIFI_SSID:-\$_E2E_WIFI_SSID_DEFAULT}}\"")
        assertThat(prod).contains("_E2E_WIFI_SSID_DEFAULT=\"mimiwifi\"")
        assertThat(prod).contains("_E2E_WIFI_PASS_DEFAULT=\"MimiWifi24pass\"")
        assertThat(prod).contains("lab-vsol-e2e-24")
        assertThat(prod).contains("LabVsolWifi24!")
        ...
        assertThat(prod).doesNotContain("tr069-e2e-mk-ping.sh")
        assertThat(prod).doesNotContain("MikroTik ping validation failed")
        assertThat(text).doesNotContain("tr069-e2e-mk-ping.sh")
        assertThat(text).contains("--wifi-ssid)")
        assertThat(text).contains("--wifi-pass)")
        assertThat(text).contains("E2E_WIFI_SSID_5=\"\${E2E_WIFI_SSID} - 5G\"")
        assertThat(text).doesNotContain("E2E_WIFI_SSID_5:-")
        assertThat(prod).contains("--wifi-ssid)")
        assertThat(prod).contains("--wifi-pass)")
        assertThat(prod).contains("E2E_WIFI_SSID_5=\"\${E2E_WIFI_SSID} - 5G\"")
        assertThat(prod).contains("¿Ejecutar hard cleanup ahora?")
        assertThat(prod).contains("--cleanup-mode)")
        assertThat(prod).contains("ask|auto|skip")
        assertThat(prod).contains("--ask-cleanup")
        assertThat(prod).contains("--auto-cleanup")
        assertThat(prod).contains("--no-cleanup")
        assertThat(text).contains("¿Ejecutar hard cleanup ahora?")
        assertThat(text).contains("--cleanup-mode)")
        assertThat(text).contains("ask|auto|skip")
        assertThat(text).contains("--ask-cleanup")
        assertThat(text).contains("--auto-cleanup")
        assertThat(text).contains("--no-cleanup")
        assertThat(text).contains("post cleanup skipped (user)")
        assertThat(prod).contains("post cleanup skipped (user)")
    ...
    fun `local core espresso script prints wifi before cleanup and uses long passphrase`() {
        val text = script.readText()
        val wifiBlock = text.indexOf("== WiFi credentials (before cleanup) ==")
        ...
        assertThat(text).contains("_E2E_WIFI_PASS_DEFAULT=\"MimiWifi24pass\"")
        assertThat("MimiWifi24pass".length).isAtLeast(8)
        assertThat("LabVsolWifi24!".length).isAtLeast(8)
